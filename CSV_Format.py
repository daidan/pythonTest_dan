...
key_form_header() find which form header is the key connections of different fragments in excel
'''
...
def key_form_header(csvReadPath):
    ...
    for i in range(0, len(form_header)):
        ...
        for j in range(i+1, len(form_header)):
            ...
            if marks:
                print('Subset of form header {} and {}'.format(form_header[i],form_header[j]))
...
'''
improve input excel file format
...
'''
judge the similarly of gen_output and output(ground true)
'''
def evelMetric(groundTruePath, generatePath):
...
if __name__ == '__main__':
    ...
    evelMetric(groundTruePath, csvSavePath)
